...
class CommentCRUD implements CRUD{
    ...
    {
        $sql = "SELECT comment_id,comment_text,comment_time,mentor_id,intern_id, CONCAT_WS(' ',users.name, users.lastname) AS intern_name 
        FROM (comments INNER JOIN users ON comments.intern_id = users.user_id) 
